fullParticipationTable <- function(long_data, params){
  ...
  #A1. "One" Way Splits 
  #A1.A Overall
  #temp2 <- p.state[[1]][p.state[[1]]$YEAR %in% c(2019,2021),]
  #temp <- long_data[long_data$YEAR %in% c(2019, 2021),]
  temp <- temp[!is.na(temp$SCALE_SCORE),]
  sum(temp$SCALE_SCORE == 100)
  temp <- temp[temp$SCALE_SCORE == 100,]
  table(temp$YEAR)
  ...
  p.state[[length(p.state)+1]] <- long_data[,
                             list(LEVEL               = "State",
                                  ...
                                  MEAN_SCALE_SCORE    = NA, #mean(SCALE_SCORE, na.rm=TRUE),
                                  MEDIAN_SCALE_SCORE  = NA#, #median(SCALE_SCORE, na.rm=TRUE)#,
                                  #PERCENT_PROF        =
