getQuantcut <- function (score, quantiles = 0:10/10) {
  ...
  tmp.cuts <- .quantcut(score, q=quantiles, right=FALSE, dig.lab=min(tmp.digits, max(nchar(score))))
  ...
  tmp.lookup <- data.table(seq(length(levs)), t(mkpairs(strsplit(levs, "[^0-9+\\.\\-]+"))))
